...
- Lists visible Easy Apply Dice search postings in the popup and opens selected postings into their application flows.
...
| Dice | Job-detail pages, using `JobPosting` JSON-LD when available. Search result pages can list visible Easy Apply postings and open selected application flows. |
...
The extension reads the job page you are viewing. On Dice search results, it can open selected visible Easy Apply postings from the current page in new tabs and click the detail page's Easy Apply link. It does not crawl job boards or submit applications.
